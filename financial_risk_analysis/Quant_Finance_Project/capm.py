...
# import market_data
import Quant_Finance_Project.market_data as market_data
...
def plot_normalized_timeseries(rics, from_date='aaaa-mm-dd', to_date='aaaa-mm-dd', base_value=100):
    """
...
class model:
    ...
    def plot_timeseries(self, secondary_y = True):
        ...
        ax2 = self.timeseries.plot(kind='line', x='date', y='close_y' , color='red', secondary_y= secondary_y, ax=ax, grid=True, label=self.security)
        ax1.legend(loc=2)
        ax2.legend(loc=1)
        plt.show()
        
    def compute_linear_reg(self, target_column = 'returns'):
        if target_column == 'returns':
            self.x = self.timeseries['return_x'].values
            self.y = self.timeseries['return_y'].values
        elif target_column == 'closing':
            self.x = self.timeseries['close_x'].values
            self.y = self.timeseries['close_y'].values
        # Lineal Regression 
    ...
    def plot_linear_reg(self, ax=None):
        ...
        str_compacto = self.security + ' vs ' + self.benchmark  + ' corr: ' + f"{self.correlation:.3f}" + \
